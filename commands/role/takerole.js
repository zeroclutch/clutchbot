    ...
    run: function(msg, args) {
      ...
      if(userID && roleName) {
        ...
        if(!user.hasRole(roleID)) {
            msg.channel.send('User does not have this role.')
            return
        }
        ...
        if(msg.member.highestRole.comparePositionTo(msg.guild.roles.find(role => role.id === roleID)) <= 0) {
            msg.channel.send('Sorry, you don\'t have the permissions to remove this role.')
            return
        }

        // add role
        user.removeRole(roleID).then(()=>{
           msg.channel.send(`${user.displayName} no longer has the "${roleName}" role.`);
         })
         .catch(err => {
           console.error(err);
        ...
        msg.channel.send(`Incorrect usage of this command. Use \`${msg.prefix}help takerole\``)
      }
    }
  }